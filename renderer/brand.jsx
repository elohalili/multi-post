...
function brushArc(cx, cy, r, a0, a1, opt = {}) {
  ...
  for (let i = 0; i <= N; i++) {
    ...
    const w = Math.max(0.8, env), ca = Math.cos(a * D2R), sa = Math.sin(a * D2R);
    const px = cx + r * ca, py = cy + r * sa;
    outer.push([px + (w / 2) * ca, py + (w / 2) * sa]);
    inner.push([px - (w / 2) * ca, py - (w / 2) * sa]);
  }
  let d = `M ${outer[0][0].toFixed(2)} ${outer[0][1].toFixed(2)} `;
  for (let i = 1; i < outer.length; i++) d += `L ${outer[i][0].toFixed(2)} ${outer[i][1].toFixed(2)} `;
  for (let i = inner.length - 1; i >= 0; i--) d += `L ${inner[i][0].toFixed(2)} ${inner[i][1].toFixed(2)} `;
  return d + "Z";
}
function brushHead(cx, cy, r, aEnd, dirSign = 1, opt = {}) {
  const { len = 15, wing = 13 } = opt;
  const ca = Math.cos(aEnd * D2R), sa = Math.sin(aEnd * D2R);
  const tx = -sa * dirSign, ty = ca * dirSign;
  const bx = cx + r * ca - tx * (len * 0.35), by = cy + r * sa - ty * (len * 0.35);
  ...
  return `M ${tip[0].toFixed(2)} ${tip[1].toFixed(2)} ` +
...
function BrushMark({ size = 36, radius = 0.26, bg = "#33383f", ink = "#ffffff", mark = true, ring = false }) {
  const arcs = [
    { r: 37, a0: 202, a1: 338 },
    { r: 37, a0: 22, a1: 158 },
  ];
    ...
    <div style={{
      ...
      <svg width={size} height={size} viewBox="0 0 100 100">
        <g fill={ink}>
          {arcs.map((a, i) => (
            <g key={i}>
              <path d={brushArc(50, 50, a.r, a.a0, a.a1, { seed: i ? 3 : 1 })} />
      ...
      {mark && (
        <svg width={size * 0.46} height={size * 0.46} viewBox="0 0 512 512" style={{ position: "absolute" }}>
...
function Wordmark({ color = "var(--text)", sub = "var(--text-3)", size = 19 }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", lineHeight: 1, gap: 3 }}>
      <span style={{ fontWeight: 700, fontSize: size, letterSpacing: "-0.02em", color }}>Multi&#8209;post</span>
      <span style={{ fontFamily: "var(--mono)", fontWeight: 500, fontSize: 9.5, letterSpacing: "0.22em", textTransform: "uppercase", color: sub }}>by elo&sup2;</span>
    </div>
  );
}

// ── stroke icon set (1.6px, currentColor, 24 box) ───────────────────────────
const S = (p) => ({ width: p.size || 18, height: p.size || 18, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: p.w || 1.7, strokeLinecap: "round", strokeLinejoin: "round" });
const Ic = {
  search: (p = {}) => (<svg {...S(p)}><circle cx="11" cy="11" r="7" /><path d="M21 21l-4.3-4.3" /></svg>),
  plus: (p = {}) => (<svg {...S(p)}><path d="M12 5v14M5 12h14" /></svg>),
  x: (p = {}) => (<svg {...S(p)}><path d="M6 6l12 12M18 6L6 18" /></svg>),
  check: (p = {}) => (<svg {...S(p)}><path d="M20 6L9 17l-5-5" /></svg>),
  chevDown: (p = {}) => (<svg {...S(p)}><path d="M6 9l6 6 6-6" /></svg>),
  chevRight: (p = {}) => (<svg {...S(p)}><path d="M9 6l6 6-6 6" /></svg>),
  image: (p = {}) => (<svg {...S(p)}><rect x="3" y="3" width="18" height="18" rx="3" /><circle cx="8.5" cy="8.5" r="1.6" /><path d="M21 15l-5-5L5 21" /></svg>),
  upload: (p = {}) => (<svg {...S(p)}><path d="M12 16V4M7 9l5-5 5 5" /><path d="M5 16v3a1 1 0 001 1h12a1 1 0 001-1v-3" /></svg>),
  trash: (p = {}) => (<svg {...S(p)}><path d="M4 7h16M9 7V5a1 1 0 011-1h4a1 1 0 011 1v2M6 7l1 13a1 1 0 001 1h8a1 1 0 001-1l1-13" /></svg>),
  grip: (p = {}) => (<svg {...S(p)}><circle cx="9" cy="6" r="1.2" fill="currentColor" stroke="none" /><circle cx="15" cy="6" r="1.2" fill="currentColor" stroke="none" /><circle cx="9" cy="12" r="1.2" fill="currentColor" stroke="none" /><circle cx="15" cy="12" r="1.2" fill="currentColor" stroke="none" /><circle cx="9" cy="18" r="1.2" fill="currentColor" stroke="none" /><circle cx="15" cy="18" r="1.2" fill="currentColor" stroke="none" /></svg>),
  star: (p = {}) => (<svg {...S(p)} fill={p.fill || "none"}><path d="M12 3.5l2.6 5.3 5.9.9-4.3 4.1 1 5.8-5.2-2.7-5.2 2.7 1-5.8L3.5 9.7l5.9-.9z" /></svg>),
  folder: (p = {}) => (<svg {...S(p)}><path d="M3 7a2 2 0 012-2h4l2 2.5h8a2 2 0 012 2V18a2 2 0 01-2 2H5a2 2 0 01-2-2z" /></svg>),
  link: (p = {}) => (<svg {...S(p)}><path d="M10 14a5 5 0 007.5.5l3-3a5 5 0 00-7-7l-1.5 1.5" /><path d="M14 10a5 5 0 00-7.5-.5l-3 3a5 5 0 007 7L12 18" /></svg>),
  ext: (p = {}) => (<svg {...S(p)}><path d="M14 4h6v6M20 4l-9 9" /><path d="M18 14v5a1 1 0 01-1 1H5a1 1 0 01-1-1V7a1 1 0 011-1h5" /></svg>),
  user: (p = {}) => (<svg {...S(p)}><circle cx="12" cy="8" r="4" /><path d="M4 21a8 8 0 0116 0" /></svg>),
  refresh: (p = {}) => (<svg {...S(p)}><path d="M21 12a9 9 0 11-3-6.7M21 4v4h-4" /></svg>),
  spark: (p = {}) => (<svg {...S(p)}><path d="M12 3l1.7 4.8L18 9.5l-4.3 1.7L12 16l-1.7-4.8L6 9.5l4.3-1.7z" /></svg>),
  arrowR: (p = {}) => (<svg {...S(p)}><path d="M4 12h15M13 6l6 6-6 6" /></svg>),
  alert: (p = {}) => (<svg {...S(p)}><path d="M12 8v5M12 16.5v.5" /><circle cx="12" cy="12" r="9" /></svg>),
  lock: (p = {}) => (<svg {...S(p)}><rect x="5" y="11" width="14" height="9" rx="2" /><path d="M8 11V8a4 4 0 018 0v3" /></svg>),
  clock: (p = {}) => (<svg {...S(p)}><circle cx="12" cy="12" r="9" /><path d="M12 7v5l3 2" /></svg>),
  dots: (p = {}) => (<svg {...S(p)}><circle cx="5" cy="12" r="1.4" fill="currentColor" stroke="none" /><circle cx="12" cy="12" r="1.4" fill="currentColor" stroke="none" /><circle cx="19" cy="12" r="1.4" fill="currentColor" stroke="none" /></svg>),
  copy: (p = {}) => (<svg {...S(p)}><rect x="9" y="9" width="11" height="11" rx="2" /><path d="M5 15V5a2 2 0 012-2h8" /></svg>),
  pen: (p = {}) => (<svg {...S(p)}><path d="M16.5 3.5a2.1 2.1 0 013 3L7 19l-4 1 1-4z" /></svg>),
};
